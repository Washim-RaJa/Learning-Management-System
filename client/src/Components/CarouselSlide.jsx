...
const CarouselSlide = ({ image, title, description, slideNumber, totalSlides }) => {
    ...
    return (
        <div id={`slide${slideNumber}`} className="carousel-item relative w-full">
            <div className="flex flex-col items-center justify-center gap-y-10 px-[15%]">
                <img src={image} className="w-40 rounded-full border-2 border-gray-400" />
                <p className="text-sm sm:text-xl mt-16 text-center text-gray-200">
                    {description}
                </p>
                <h3 className="sm:text-2xl font-semibold">{title}</h3>
